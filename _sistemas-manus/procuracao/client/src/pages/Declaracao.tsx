...
function TelaConfirmacao({ nome, onNovo }: { nome: string; onNovo: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <p className="text-slate-600 text-sm leading-relaxed max-w-sm mb-2">Seus dados foram recebidos. O advogado preparará a declaração e entrará em contato.</p>
...
export default function Declaracao() {
  ...
  const dataValue = watch("dataDeclaracao") ?? "";
  ...
  const handleData = useCallback((e: React.ChangeEvent<HTMLInputElement>) => setValue("dataDeclaracao", maskDate(e.target.value), { shouldValidate: true }), [setValue]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const response = await fetch("/api/declaracao", {
        method: "POST",
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg, #0f1e3d 0%, #1a2f5a 40%, #1e3a6e 100%)" }}>
        ...
        <div className="container py-4 flex items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs">Documento jurídico · Declaração de Hipossuficiência</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container pt-10 pb-6">
        <div className="text-center max-w-xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-400/20 text-amber-300 text-xs font-medium mb-4 tracking-widest uppercase">
            <FileText size={12} />Declaração de Hipossuficiência
          </div>
          ...
          <p className="text-white/60 text-sm leading-relaxed">
            Preencha o formulário abaixo. O advogado responsável preparará a declaração e entrará em contato.
          </p>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto rounded-2xl shadow-2xl overflow-hidden" style={{ background: "#f8f7f4", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
          ...
          {enviado ? <TelaConfirmacao nome={nomeEnviado} onNovo={() => { reset(); setEnviado(false); setNomeEnviado(""); }} /> : (
            <div className="p-6 md:p-10">
              <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-200">
                ...
                <div>
                  <h3 className="font-serif text-[#1a2744] text-xl font-semibold">Declaração de Hipossuficiência</h3>
              ...
              <form onSubmit={handleSubmit(onSubmit)} noValidate autoComplete="off">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                  ...
                  <Field label="Quadra" hint="Deixe em branco se não houver">
                    <input {...register("quadra")} placeholder="Ex.: 10" className={inputClass} />
                  </Field>
                  <Field label="Lote" hint="Deixe em branco se não houver">
                ...
                <SectionTitle>III — Data da Declaração</SectionTitle>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                  <Field label="Data" error={errors.dataDeclaracao?.message} required hint="Formato: DD/MM/AAAA">
                    <input value={dataValue} onChange={handleData} placeholder="Ex.: 08/06/2026" className={errors.dataDeclaracao ? inputErrorClass : inputClass} inputMode="numeric" maxLength={10} />
                  </Field>
                </div>
          ...
          {!enviado && (
            <div className="px-6 md:px-10 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between flex-wrap gap-2">
              <p className="text-xs text-slate-400">Declaração de Hipossuficiência · Gratuidade de Justiça</p>
      ...
      <footer className="border-t border-white/10 py-6">
        <div className="container text-center">
          <p className="text-white/30 text-xs">Declaração de Hipossuficiência</p>
        </div>
      </footer>
    </div>
  );
