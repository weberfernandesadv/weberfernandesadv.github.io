...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <div className="flex items-center gap-2 mb-5 mt-2"><div className="h-px flex-1 bg-slate-200" /><span className="text-xs font-semibold text-slate-400 uppercase tracking-widest px-2">{children}</span><div className="h-px flex-1 bg-slate-200" /></div>;
}
...
function TelaConfirmacao({ nome, nomeModelo, onNovo }: { nome: string; nomeModelo: string; onNovo: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <p className="text-slate-600 text-sm leading-relaxed max-w-sm mb-2">Seus dados foram recebidos. O advogado preparará o documento e entrará em contato.</p>
...
export default function ModeloDinamico() {
  ...
  const [modelo, setModelo] = useState<{ nome: string; descricao?: string | null } | null>(null);
  const [modeloError, setModeloError] = useState<string | null>(null);
  const [modeloLoading, setModeloLoading] = useState(true);

  // Busca os dados do modelo pelo slug
  useEffect(() => {
    if (!slug) return;
    setModeloLoading(true);
    fetch(`/api/modelo/${slug}`)
      .then(r => {
        if (!r.ok) throw new Error("Modelo não encontrado ou inativo");
        return r.json();
      })
      .then(data => { setModelo(data); setModeloLoading(false); })
      .catch(err => { setModeloError(err.message); setModeloLoading(false); });
  }, [slug]);
  ...
  const dataValue = watch("dataAssinatura") ?? "";
  ...
  const handleData = useCallback((e: React.ChangeEvent<HTMLInputElement>) => setValue("dataAssinatura", maskDate(e.target.value), { shouldValidate: true }), [setValue]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const response = await fetch(`/api/modelo/${slug}`, {
        method: "POST",
  ...
  if (modeloLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(160deg, #0f1e3d 0%, #1a2f5a 40%, #1e3a6e 100%)" }}>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(160deg, #0f1e3d 0%, #1a2f5a 40%, #1e3a6e 100%)" }}>
        <div className="text-center px-6">
          ...
          <h2 className="text-white text-xl font-semibold mb-2">Formulário não encontrado</h2>
          <p className="text-white/50 text-sm">Este link pode estar inativo ou expirado. Entre em contato com o advogado.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg, #0f1e3d 0%, #1a2f5a 40%, #1e3a6e 100%)" }}>
        ...
        <div className="container py-4 flex items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs">Documento jurídico · {modelo.nome}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container pt-10 pb-6">
        <div className="text-center max-w-xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-400/20 text-amber-300 text-xs font-medium mb-4 tracking-widest uppercase">
            <FileText size={12} />{modelo.nome}
          </div>
          <h2 className="font-serif text-white text-3xl md:text-4xl font-semibold leading-snug mb-3">
            Preencha seus dados<br /><span className="text-amber-400">com segurança</span>
          </h2>
          {modelo.descricao && (
            <p className="text-white/60 text-sm leading-relaxed">{modelo.descricao}</p>
          )}
          {!modelo.descricao && (
            <p className="text-white/60 text-sm leading-relaxed">
              Preencha o formulário abaixo com seus dados pessoais. O advogado responsável preparará o documento.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="container pb-16">
        <div className="max-w-3xl mx-auto rounded-2xl shadow-2xl overflow-hidden" style={{ background: "#f8f7f4", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
          ...
          {enviado ? <TelaConfirmacao nome={nomeEnviado} nomeModelo={modelo.nome} onNovo={() => { reset(); setEnviado(false); setNomeEnviado(""); }} /> : (
            <div className="p-6 md:p-10">
              <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-200">
                ...
                <div>
                  <h3 className="font-serif text-[#1a2744] text-xl font-semibold">{modelo.nome}</h3>
              ...
              <form onSubmit={handleSubmit(onSubmit as unknown as Parameters<typeof handleSubmit>[0])} noValidate autoComplete="off">
                <SectionTitle>I — Identificação do Outorgante</SectionTitle>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                  ...
                  <Field label="CPF" error={errors.cpf?.message} required hint="Formato: xxx.xxx.xxx-xx">
                ...
                <SectionTitle>III — Data de Assinatura</SectionTitle>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                  <Field label="Data de Assinatura" error={errors.dataAssinatura?.message} required hint="Formato: DD/MM/AAAA">
                    <input value={dataValue} onChange={handleData} placeholder="Ex.: 08/06/2026" className={errors.dataAssinatura ? inputErrorClass : inputClass} inputMode="numeric" maxLength={10} />
                  </Field>
                </div>

                <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-50 border border-blue-200 mb-8">
                  <AlertCircle size={16} className="text-blue-600 mt-0.5 flex-shrink-0" />
                  <p className="text-xs text-blue-800 leading-relaxed">Seus dados serão enviados com segurança ao advogado responsável, que preparará o documento.</p>
                </div>
          ...
          {!enviado && (
            <div className="px-6 md:px-10 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between flex-wrap gap-2">
              <p className="text-xs text-slate-400">{modelo.nome}</p>
      ...
      <footer className="border-t border-white/10 py-6">
        <div className="container text-center">
          <p className="text-white/30 text-xs">{modelo.nome}</p>
        </div>
      </footer>
    </div>
  );
