...
const schema = z.object({
  ...
  dataContrato: z.string().regex(/^\d{2}\/\d{2}\/\d{4}$/, "Data inválida — use DD/MM/AAAA"),
});
...
function TelaConfirmacao({ nome, onNovo }: { nome: string; onNovo: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <p className="text-slate-600 text-sm leading-relaxed max-w-sm mb-2">Seus dados foram recebidos. O advogado preparará o contrato e entrará em contato.</p>
...
export default function Contrato() {
  ...
  const dataContratoValue = watch("dataContrato") ?? "";
  ...
  const handleDataContrato = useCallback((e: React.ChangeEvent<HTMLInputElement>) => setValue("dataContrato", maskDate(e.target.value), { shouldValidate: true }), [setValue]);
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg, #0f1e3d 0%, #1a2f5a 40%, #1e3a6e 100%)" }}>
        ...
        <div className="container py-4 flex items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs">Documento jurídico · Contrato de Honorários</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container pt-10 pb-6">
        <div className="text-center max-w-xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-400/20 text-amber-300 text-xs font-medium mb-4 tracking-widest uppercase">
            <FileText size={12} />Contrato de Honorários Advocatícios
          </div>
          ...
          <p className="text-white/60 text-sm leading-relaxed">
            Preencha o formulário abaixo com seus dados pessoais e a data de assinatura. O advogado responsável completará as demais informações e preparará o contrato.
          </p>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto rounded-2xl shadow-2xl overflow-hidden" style={{ background: "#f8f7f4", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
          ...
          {enviado ? <TelaConfirmacao nome={nomeEnviado} onNovo={() => { reset(); setEnviado(false); setNomeEnviado(""); }} /> : (
            <div className="p-6 md:p-10">
              <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-200">
                ...
                <div>
                  <h3 className="font-serif text-[#1a2744] text-xl font-semibold">Contrato de Honorários Advocatícios</h3>
              ...
              <form onSubmit={handleSubmit(onSubmit as unknown as Parameters<typeof handleSubmit>[0])} noValidate autoComplete="off">
                <SectionTitle>I — Identificação do Contratante</SectionTitle>
                ...
                <SectionTitle>Data de Assinatura</SectionTitle>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                  <Field label="Data de Assinatura do Contrato" error={errors.dataContrato?.message} required hint="Formato: DD/MM/AAAA">
                    <input value={dataContratoValue} onChange={handleDataContrato} placeholder="Ex.: 08/06/2026" className={errors.dataContrato ? inputErrorClass : inputClass} inputMode="numeric" maxLength={10} />
                  </Field>
                </div>

                <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-50 border border-blue-200 mb-8">
                  <AlertCircle size={16} className="text-blue-600 mt-0.5 flex-shrink-0" />
                  <p className="text-xs text-blue-800 leading-relaxed">Seus dados serão enviados com segurança ao advogado responsável, que completará as informações do processo e preparará o contrato.</p>
                </div>
          ...
          {!enviado && (
            <div className="px-6 md:px-10 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between flex-wrap gap-2">
              <p className="text-xs text-slate-400">Contrato de Honorários Advocatícios</p>
      ...
      <footer className="border-t border-white/10 py-6">
        <div className="container text-center">
          <p className="text-white/30 text-xs">Contrato de Honorários Advocatícios</p>
        </div>
      </footer>
    </div>
  );
